...
const provinceChangeCallback = async () => {
  ...
  if (provinceId) {
    formData.value.provinceName =
      areaInfo.provinceList.find((item) => item.id === provinceId)?.name || ''
    resetCity()
    resetDistrict()
...
const cityChangeCallback = async () => {
  ...
  if (cityId) {
    formData.value.cityName =
      areaInfo.cityList.find((item) => item.id === cityId)?.name || ''
    resetDistrict()
...
const districtChangeCallback = async () => {
  ...
  if (districtId) {
    formData.value.districtName =
      areaInfo.districtList.find((item) => item.id === districtId)?.name || ''
  }
}
// 区域清除回调
